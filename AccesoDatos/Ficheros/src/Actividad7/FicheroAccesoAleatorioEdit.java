...
public class FicheroAccesoAleatorioEdit {
     ...
     */
    FicheroAccesoAleatorioEdit(String nomFich, List<Pair<String, Integer>> campos)
            throws IOException {
        this.campos = campos;
    ...
    /**
     * Obtiene el valor de un campo de un registro específico.
     * @param pos  La posición del registro (0 para el primer registro).
     * @param campoNombre El nombre del campo a obtener.
     * @return El valor del campo como String, con la misma longitud fija que se almacenó.
     * @throws IOException Si ocurre un error de E/S o el registro/campo no existe.
     */
    public String obtenerValorCampo(long pos, String campoNombre) throws IOException {
        if (pos < 0 || pos >= numReg) {
            throw new IllegalArgumentException("Posición de registro fuera de rango.");
        }

        try (RandomAccessFile raf = new RandomAccessFile(f, "r")) {
            // Posiciona el puntero al inicio del registro deseado
            raf.seek(pos * longReg);

            // Recorremos los campos hasta encontrar el que nos interesa
            for (Pair<String, Integer> campo : campos) {
                String nombre = campo.getKey();
                int longitud = campo.getValue();
                byte[] buffer = new byte[longitud];
                raf.read(buffer);  // Leemos exactamente la longitud del campo

                if (nombre.equals(campoNombre)) {
                    return new String(buffer, StandardCharsets.UTF_8);
                }
            }
        }

        // Si no se encontró el campo, lanzamos excepción
        throw new IllegalArgumentException("El campo '" + campoNombre + "' no existe en el registro.");
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            FicheroAccesoAleatorioEdit faa = new FicheroAccesoAleatorioEdit("fic_acceso_ aleat.dat", campos);
            ...
            faa.insertar(reg, 1);
            try {
                String nombreRegistro0 = faa.obtenerValorCampo(0, "NOMBRE"); // Primer registro
                System.out.println("Registro 0, campo NOMBRE: '" + nombreRegistro0 + "'");

                String cpRegistro1 = faa.obtenerValorCampo(1, "CP"); // Segundo registro
                System.out.println("Registro 1, campo CP: '" + cpRegistro1 + "'");

            } catch (IOException e) {
                System.err.println("Error leyendo el registro: " + e.getMessage());
            }
